...
public class permission_dao {
    ...
    public void save_or_update_permissions(int draw_id, int user_id, boolean can_read, boolean can_write) {
        ...
        if (count > 0) {
            ...
            int rowsAffected = jdbcTemplate.update(updateSql, can_read, can_write, draw_id, user_id);
            System.out.println("UPDATE ejecutado. Filas afectadas: " + rowsAffected);
        } else {
            String insertSql = "INSERT INTO permissios (draw_id, user_id, can_read, can_write) VALUES (?, ?, ?, ?)";
            int rowsAffected = jdbcTemplate.update(insertSql, draw_id, user_id, can_read, can_write);
            System.out.println("INSERT ejecutado. Filas afectadas: " + rowsAffected);
        }
    }
    ...
    public Optional<permissions> get_permissions_for_user(int draw_id, int user_id) {
        String sql = "SELECT draw_id, user_id, can_read, can_write FROM permissios WHERE draw_id = ? AND user_id = ?";
        try {
